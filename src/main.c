...
int main(){
    ...
    int col_multivector[7] = {1,3,4,8,12,16,32};
    ...
    for(int i = 0; i < NFILES; i++){
        ...
        for(int j = 0; j < 7; j++){
            ...
            for(int k = 0; k < NREPETITIONS; k++){
                ...
                prepara_risultato(converted_csr_matrix.m, multivector.n,&result1);
                //printf("ellpack\n");
                //timeSumEllpackOmp += optimized_ellpack_product(converted_ellpack_matrix,multivector, &result2);
                //printf("cuda\n");
                timeSumCsrCuda += calcola_prodotto_csr_cuda(converted_csr_matrix, multivector, &result1);
            ...
            fprintf(resultsCsrCuda,"%s, %d, %d, %d, %d, %f, %f\n",matFiles[i],converted_csr_matrix.m, converted_csr_matrix.n, converted_csr_matrix.nz, col_multivector[j], timeSumCsrCuda/NREPETITIONS,((2*col_multivector[j]/pow(10,9))*converted_csr_matrix.nz)/(timeSumCsrCuda/NREPETITIONS));
